;;;; Reversible experiment: reduce temporal oversampling to ten anchors.
...
(let* ((source (experiment-read
       ...
       (output (merge-pathnames "temporal-10-windows.sexp" *experiment-root*)))
  (with-open-file (stream output :direction :output :if-exists :supersede
                                 :if-does-not-exist :create)
    (let ((*print-pretty* t) (*print-circle* nil))
      (prin1 (list :schema 'evidence-windows/1 :records records) stream)
      (terpri stream)))
  (unless (= 200 (length records))
    (error "Expected 200 temporal-anchor records; found ~D." (length records)))
