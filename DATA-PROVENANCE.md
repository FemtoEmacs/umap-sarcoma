...
The UMAP causal overlay is generated leave-campaign-out with the validated
CINEMA-OT adaptation. For each held-out campaign, high-flux observations have
`F10.7 >= 140` and low-flux observations have `F10.7 <= 100`. The displayed
quantity is the context-matched high-minus-low effect on `VIPN - SF99`, in
m/s. A fold is marked unsupported when the smaller treatment group has less
than 20% of the observations in the larger group. The current estimator does
not supply a validated confidence interval.
